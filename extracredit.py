...
# import cs1lib and random integers
from cs1lib import *
import random
...
BALL_MOVE = 2  # how many pixels it is moving
...
SPEED = 2.5  # constant to make ball faster
...
def graphics_window():
    ...
    clear()
    disable_stroke()
    img = load_image("drake1.png")
    draw_image(img, 200, 200)
    ...
    ball_x_velocity = random.uniform(-3,3)  # sets initial x velocity of ball, random!
    ball_y_velocity = random.uniform(-2,2)  # sets initial y velocity of ball, random!
    ...
    while not window_closed():
        ...
        if is_key_pressed(" "):
            ...
            ball_x_velocity = random.uniform(-3,3)  # random!
            ball_y_velocity = random.uniform(-2,2)  # random!
            request_redraw()
            clear()
        ...
        if contact_paddle_1(ball_x, ball_y, moving_y1):
            ball_x_velocity = BALL_MOVE * SPEED

        if contact_paddle_2(ball_x, ball_y, moving_y2):
            ball_x_velocity = ball_x_velocity * BOUNCE * SPEED

        if contact_top_wall(ball_y):
            ball_y_velocity = BALL_MOVE * SPEED

        if contact_bottom_wall(ball_y):
            ball_y_velocity = ball_y_velocity * BOUNCE * SPEED
